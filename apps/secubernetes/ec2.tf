...
resource "aws_instance" "win" {
  ...
  instance_type               = local.ec2_instance_type
  associate_public_ip_address = true
  key_name                    = aws_key_pair.y.key_name
  root_block_device {
    volume_size = 300
  }
  volume_tags = merge(
    local.common_tags,
    {
      Name = "win-volume-${count.index}"
  })
  tags = merge(
    local.common_tags,
    {
      Name = "win-${data.aws_region.eu.name}-${count.index}.${local.dns_hosted_zone_name}"
  })
  iam_instance_profile = data.aws_iam_instance_profile.instance_profile.name
}

resource "aws_route53_record" "win-ssh" {
  count   = length(module.vpc.public_subnets)
  zone_id = data.aws_route53_zone.iot.id
  name    = "win-${data.aws_region.eu.name}-${count.index}.${local.dns_hosted_zone_name}"
  type    = "A"
  ttl     = "300"
  records = [aws_instance.win[count.index].public_ip]
}

output "eu-win" {
  value = aws_route53_record.win-ssh[*].name
}
