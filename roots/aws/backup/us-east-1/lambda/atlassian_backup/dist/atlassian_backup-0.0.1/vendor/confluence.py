...
def conf_backup(account, username, token, attachments, folder):
    ...
    if file_name != "None":
        ...
        print(
            "Backup file can also be downloaded from "
            + url
            + "/wiki/download/"
            + file_name
        )
...
def main():
    ...
    parser.add_argument(
        "-s", "--site", help="Your site/account name <account>.atlassian.net"
    )
    parser.add_argument(
        "-u", "--user", help="Your email address for the jira account with admin rights"
    )
    parser.add_argument("-t", "--token", help="API token for the user account")
    parser.add_argument(
        "-a",
        "--attachments",
        help="[Y/y] to download with attachments or [N/n] to download without",
    )
    parser.add_argument(
        "-f",
        "--folder",
        help="Destination folder for the backup file eg. /folder/sub-folder/",
    )
